<?php
/**
 * Name:
 * Date:
 */

require_once('../app/config/config.php');
require_once('../app/functions.php');
$userSigned = $user->isSignedIn();

//if not logged in redirect to login page
ifLoggedIn(BASE_LINK . 'homecontroller/home', $userSigned);

?>

<div class="container">
    <div class="row">
        <div class="col-md-6">

            <h2>Home Maintenance Manager</h2>
            <hr>
            <p>Lorem ipsum dolor sit amet, consectetuer adipiscing elit. Aenean commodo ligula eget dolor. Aenean massa.</p>

            <p>
                sociis natoque penatibus et magnis dis parturient montes, nascetur ridiculus mus. Donec quam felis,
                ultricies nec, pellentesque eu, pretium quis, sem. Nulla consequat massa quis enim. Donec pede justo,
                vel, aliquet nec, vulputate
            </p>
            <br>
            <div class="col-xs-6 col-md-6">
                <h4><a class="btn btn-dark btn-block btn-lg" href='/home_maintenance_manager/public/usercontroller/signup'>Sign Up</a></h4>
                <h4><a class="btn btn-dark btn-block btn-lg" href='/home_maintenance_manager/public/usercontroller/signin'>Sign In</a></h4>
            </div>
        </div>
        <div class="col-md-6">
            <p>Lorem Ipsum</p>
        </div>
    </div>

</div>
